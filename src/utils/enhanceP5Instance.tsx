...
function enahnceP5Instance(p5_instance: P5InstanceType) {
  ...
  function _intersectLines(
    ...
    const x1 = seg1Start.x,
      y1 = seg1Start.y;
    const x2 = seg1End.x,
      y2 = seg1End.y;
    const x3 = seg2Start.x,
      y3 = seg2Start.y;
    const x4 = seg2End.x,
      y4 = seg2End.y;
    ...
    const deltaX2 = x4 - x3,
      deltaY2 = y4 - y3;
    const denominator = deltaY2 * deltaX1 - deltaX2 * deltaY1;
    if (denominator === 0) {
      return false; // Lines are parallel
    }
    // Calculate the intersection point
    const ua = (deltaX2 * (y1 - y3) - deltaY2 * (x1 - x3)) / denominator;
    const ub = (deltaX1 * (y1 - y3) - deltaY1 * (x1 - x3)) / denominator;
    // Check if the intersection is within the bounds of the line segments
    if (!includeSegmentExtension && (ub < 0 || ub > 1)) {
      return false;
    }
    // Calculate the intersection coordinates
    const x = x1 + ua * deltaX1;
    const y = y1 + ua * deltaY1;
    return { x: x, y: y };
  }
  ...
  const Mix = {
     ...
     */
    getPigment(_color) {
      const currentLevels = _color.levels;
    ...
    blend(
      ...
      const newColor = !_color ? this.currentColor : this.getPigment(_color);
  ...
  const FF = {
     ...
     */
    genField() {
      const grid = new Array(this.num_columns); // Initialize the field array
    ...
    addStandard() {
      addField("curved", function (t, field) {
        ...
        for (let column = 0; column < FF.num_columns; column++) {
          for (let row = 0; row < FF.num_rows; row++) {
            ...
            const angle = R.map(noise_val, 0.0, 1.0, -angleRange, angleRange);
      ...
      addField("waves", function (t, field) {
        const sinrange = R.randInt(10, 15) + 5 * R.sin(t);
        const cosrange = R.randInt(3, 6) + 3 * R.cos(t);
        const baseAngle = R.randInt(20, 35);
        for (let column = 0; column < FF.num_columns; column++) {
          for (let row = 0; row < FF.num_rows; row++) {
            const angle =
      ...
      addField("seabed", function (t, field) {
        const baseSize = R.random(0.4, 0.8);
        const baseAngle = R.randInt(18, 26);
        for (let column = 0; column < FF.num_columns; column++) {
          for (let row = 0; row < FF.num_rows; row++) {
            const addition = R.randInt(15, 20);
            const angle = baseAngle * R.sin(baseSize * row * column + addition);
    ...
    plotTo(_plot, _length, _step_length, _scale) {
      if (this.isIn()) {
        ...
        for (let i = 0; i < _length / _step_length; i++) {
          ...
          const x_step = _step_length * R.cos(current_angle - plot_angle);
          const y_step = _step_length * R.sin(current_angle - plot_angle);
  ...
  function globalScale(_scale) {
    for (const s of _standard_brushes) {
      const params = B.list.get(s[0]).param;
  ...
  const B = {
    ...
    line(x1, y1, x2, y2) {
      ...
      const angle = _calculateAngle(x1, y1, x2, y2);
      B.draw(angle, false);
    },
    ...
    tip(custom_pressure = false) {
      const pressure = custom_pressure
        ? custom_pressure
        : this.calculatePressure(); // Calculate Pressure
      const alpha = this.calculateAlpha(pressure); // Calcula Alpha
    ...
    drawSpray(pressure) {
      ...
      for (let j = 0; j < iterations; j++) {
        const r = R.random(0.9, 1.1);
        const rX = r * vibration * R.random(-1, 1);
        const yRandomFactor = R.random(-1, 1);
        const rVibrationSquared = Math.pow(r * vibration, 2);
        const sqrtPart = Math.sqrt(rVibrationSquared - Math.pow(rX, 2));
        this.mask.circle(
          this.position.x + rX,
    ...
    drawMarker(pressure, vibrate = true) {
      const vibration = vibrate ? this.w * this.p.vibration : 0;
      const rx = vibrate ? vibration * R.random(-1, 1) : 0;
      const ry = vibrate ? vibration * R.random(-1, 1) : 0;
      this.mask.circle(
        this.position.x + rx,
        this.position.y + ry,
    ...
    markerTip() {
      if (this.isInsideClippingArea()) {
        ...
        const alpha = this.calculateAlpha(pressure);
  ...
  const T = {
    ...
    load() {
      for (const key of this.tips.keys()) {
        const image = loadImage(key, () => T.imageToWhite(image));
  ...
  const H = {
     ...
     */
    hatch(polygons) {
      const dist = H.hatchingParams[0];
      let angle = H.hatchingParams[1];
      const options = H.hatchingParams[2];

      // Save current stroke state
      const strokeColor = B.c,
        strokeBrush = B.name,
        strokeWeight = B.w,
      ...
      const startY = angle <= 90 && angle >= 0 ? minY : maxY;
      ...
      const r = options.rand ? options.rand : 0;
      for (let j = 0; j < gdots.length; j++) {
        const dd = gdots[j];
        const shouldDrawContinuousLine = j > 0 && options.continuous;
  ...
  class Polygon {
    ...
    intersect(line) {
      ...
      for (const s of this.sides) {
        const intersection = _intersectLines(
          line.point1,
          line.point2,
          s[0],
          s[1]
  ...
  class Plot {
    ...
    genPol(_x, _y, _scale = 1, isHatch = false) {
      ...
      const _step = B.spacing(); // get last spacing
      const vertices = [];
      const bleed = R.constrain(F.bleed_strength, 0.05, 1);
      const side = (max + min) * (isHatch ? 0.03 : F.isAnimated ? 0.25 : bleed);
      this.calcIndex(0);
      const linepoint = new Position(_x, _y);
      const numsteps = Math.round(this.length / _step);
  ...
  function drawCircle(x, y, radius, r = false) {
    ...
    const angle2 = r ? R.randInt(-5, 5) : 0;
      ...
      for (let i = 0; i < array_points.length - 1; i++) {
        if (curvature > 0 && i < array_points.length - 2) {
          ...
          if (Math.floor(a1) === Math.floor(a2)) {
            ...
            const int = _intersectLines(point1, point2, point3, point4, true);
            ...
            const s3 = (2 * Math.PI * radius * a3) / 360;
          ...
          const a = _calculateAngle(p1[0], p1[1], p2[0], p2[1]);
  ...
  function drawSpline(array_points, curvature = 0.5) {
    const p = _createSpline(array_points, curvature); // Create a new Plot-spline instance
    ...
    constructor(_v, _m, _center, dir, isFirst = false) {
      ...
      if (isFirst) {
        ...
        for (let i = 0; i < this.v.length; i++) {
          ...
          const side = p5.Vector.sub(nextVertex, currentVertex);
          const direction1 = side.copy();
          ...
          let d1 = 0;
          for (const int of F.polygon.intersect(linea)) {
